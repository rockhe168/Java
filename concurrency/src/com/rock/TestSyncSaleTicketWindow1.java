...
class TicketWindow3 implements Runnable{
    ...
    int count = 100;
    //	Object obj = new Object(); //象征着厕所的那一扇门
    public void run() {
        while (true){
            synchronized (this) { //this表示当前对象，本题中即为ticketWindow---> 如果此处用extends Tread 实现，则就不能用this，因为锁的不是同一间厕所的门
                try {
                    Thread.sleep(100);
...
public class TestSyncSaleTicketWindow1 {
